...
void doScriptsLoad()
{
    ...
    for (elementScript; elementScript; elementScript = elementScript->NextSiblingElement())
    {
        ...
        // get hotkey
        elementScript->QueryIntAttribute("hotkey" , &script.hotkey);

        // get hotkey name
        script.hotkeyName = elementScript->Attribute("hotkeyname");

        // get timer
        elementScript->QueryIntAttribute("timer" , &script.timer);

        // get timer start enabled
        elementScript->QueryIntAttribute("timerstartenabled" , &script.timerStartEnabled);

        // get show in tray menu
        elementScript->QueryIntAttribute("showintraymenu" , &script.showInTrayMenu);
...
void doTrayMenuAppendScripts(HMENU menu)
{
    ...
    for(vectorScriptsIt = vectorScripts.begin(); vectorScriptsIt != vectorScripts.end(); vectorScriptsIt++)
    {
        ...
        menuItemText << "\t";
        if (vectorScriptsIt->hotkey != 0) // check for hotkey
